...
variable "architecture" {
  ...
  validation {
    condition     = (can(regex("^Standard_[A-Z]+[0-9]+p", var.web_vm_size)) == (var.architecture == "arm64")) && (can(regex("^Standard_[A-Z]+[0-9]+p", var.monitoring_vm_size)) == (var.architecture == "arm64")) && (can(regex("^Standard_[A-Z]+[0-9]+p", var.sip_vm_size)) == (var.architecture == "arm64")) && (can(regex("^Standard_[A-Z]+[0-9]+p", var.rtp_vm_size)) == (var.architecture == "arm64")) && (can(regex("^Standard_[A-Z]+[0-9]+p", var.feature_server_vm_size)) == (var.architecture == "arm64")) && (can(regex("^Standard_[A-Z]+[0-9]+p", var.recording_vm_size)) == (var.architecture == "arm64"))
    error_message = "Architecture mismatch: architecture and every vm_size (web_vm_size, monitoring_vm_size, sip_vm_size, rtp_vm_size, feature_server_vm_size, recording_vm_size) must agree. arm64 needs Ampere sizes (Standard_D2pls_v6, Standard_D4pls_v6, ...); amd64 needs x86 sizes (Standard_F4s_v2, Standard_D2s_v3, ...). The arm64 example file sets all of them."
  }
}
