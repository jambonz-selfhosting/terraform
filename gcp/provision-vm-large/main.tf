# Main Terraform configuration for jambonz large cluster on GCP
...
# Web server HTTP/HTTPS (portal, API)
...
resource "google_compute_firewall" "monitoring" {
  name    = "${var.name_prefix}-allow-monitoring"
  network = google_compute_network.jambonz.name

  allow {
    protocol = "tcp"
    ports    = ["3000", "9080", "16686"]
  }

  source_ranges = var.allowed_http_cidr
  target_tags   = ["jambonz-monitoring"]
}

# SIP server traffic
resource "google_compute_firewall" "sip" {
  name    = "${var.name_prefix}-allow-sip"
  network = google_compute_network.jambonz.name
  ...
  target_tags   = ["jambonz-sip"]
}

# RTP server traffic
resource "google_compute_firewall" "rtp" {
  name    = "${var.name_prefix}-allow-rtp"
  network = google_compute_network.jambonz.name
  ...
  source_ranges = var.allowed_rtp_cidr
  target_tags   = ["jambonz-rtp"]
}
